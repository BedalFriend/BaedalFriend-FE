...
export default function SearchMap({
  ...
  const saveAddressHandler = () => {
    if (markerInfo.road_address_name === '') {
      const storeName = markerInfo.place_name;
      const storeAddress = markerInfo.road_address_name;
      const tempArr = { ...data };
      tempArr[`${name}`] = storeName;
      tempArr[`${address}`] = storeAddress;
      tempArr[`${roomTitle}`] = storeName;
      setData(tempArr);
      setIndex(0);
    } else {
      const storeName = markerInfo.place_name;
      const storeAddress = markerInfo.address_name;
      const tempArr = { ...data };
      tempArr[`${name}`] = storeName;
      tempArr[`${address}`] = storeAddress;
      tempArr[`${roomTitle}`] = storeName;
  ...
  return (
    <SearchST.SearchMapBox>
      ...
      >
        ...
        <SearchST.CurrentBox onClick={getCurrentPosBtn} hideMap={hideMap}>
          <img
            src={CurrentMark}
            alt=''
            style={{
